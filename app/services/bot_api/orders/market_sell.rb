  ...
  module Orders
    class MarketSell
      ...
      def call
        ...
        refusal = Lookup.untradable_refusal(exchange, ticker)
        return refusal if refusal

        exchange.set_client(api_key: api_key)
        # Default amount_type for sell is `:base` (you specify how much of the
        # asset to sell), matching the legacy MCP tool.
        effective_type = (@amount_type.presence || 'base').to_sym
        upstream = Lookup.with_dry_run(@dry_run) do
          exchange.market_sell(ticker: ticker, amount: @amount, amount_type: effective_type)
        end
