    ...
    module Probit
      class Market < BaseMarket
        ...
        def fetch_all_symbols
          ...
          Result::Failure.new("Couldn't fetch Probit symbols", RECOVERABLE.to_s)
        end
        ...
        def fetch_book(symbol)
          ...
          Result::Failure.new("Couldn't fetch Probit books", RECOVERABLE.to_s)
        end

        def fetch_all_books
          request = @caching_client.get('/api/exchange/v1/market')
          response = JSON.parse(request.body)
          return Result::Failure.new("Couldn't fetch Probit books", RECOVERABLE.to_s) unless request.status == 200
        ...
        def base_decimals(symbol)
          ...
          Result::Failure.new("Couldn't fetch Probit symbol details", RECOVERABLE.to_s)
        end
        ...
        def current_bid_ask_price(symbol)
          ...
          Result::Failure.new('Could not fetch current price from Probit', RECOVERABLE.to_s)
        end
