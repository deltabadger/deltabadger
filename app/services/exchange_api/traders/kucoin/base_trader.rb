  ...
  module Traders
    module Kucoin
      class BaseTrader < ExchangeApi::Traders::BaseTrader
        ...
        def transaction_price(symbol, price, force_smart_intervals, smart_intervals_value, is_legacy = false)
          min_price = is_legacy ? @market.minimum_quote_size(symbol) : @market.minimum_base_size(symbol)
          return min_price unless min_price.success?

          price_decimals = is_legacy ? @market.quote_decimals(symbol) : @market.base_decimals(symbol)
        ...
        def smart_volume(symbol, price, rate, force_smart_intervals, smart_intervals_value, is_legacy = false)
          volume_decimals = @market.base_decimals(symbol)
          return volume_decimals unless volume_decimals.success?

          volume = (is_legacy ? (price / rate) : price).ceil(volume_decimals.data)
