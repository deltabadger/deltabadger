...
class MarketBuyTool < ApplicationMCPTool
  ...
  property :base_asset, type: 'string', description: 'Asset symbol to buy (e.g., BTC, ETH, QQQM, AAPL). Required without bot_id'
  ...
  def perform
    ...
    currency = data[:amount_type] == 'quote' ? quote : base
    render text: "#{prefix}Market buy order #{placed(data)}: #{amount} #{currency} of #{data[:pair]}. #{detail(data)}"
  end
