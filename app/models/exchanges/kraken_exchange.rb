module Exchanges
  class KrakenExchange # rubocop:disable Metrics/ClassLength
    ...
    def get_tickers_info
      tickers_info = Rails.cache.fetch("exchange_#{@exchange.id}_info", expires_in: 1.hour) do
        ...
        return Result::Failure.new("Failed to get #{@exchange.name} tradable asset pairs") unless result.success?
    ...
    def get_order(order_id:)
      ...
      ticker = @exchange.tickers.find_by(ticker: pair)
    ...
    def get_ticker_information(base_asset_id:, quote_asset_id:) # rubocop:disable Metrics/MethodLength
      Rails.cache.fetch("exchange_#{@exchange.id}_ticker_information_#{base_asset_id}_#{quote_asset_id}", expires_in: 1.second) do # rubocop:disable Metrics/BlockLength
