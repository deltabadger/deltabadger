...
**Venue codes decide.** Read a venue's code wherever it arrives — `Exchange#response_field` covers both the HTTP-200 envelope and the JSON body of an HTTP error. Only a documented "order not found" on the cancel probe proves trade permission, and only a documented "no permission"/"invalid key" rejects; timeouts, throttles, outages and unrecognised codes are inconclusive (`Failure`). KuCoin's not-found code is undocumented, so it alone stays lenient for unknown venue codes.
