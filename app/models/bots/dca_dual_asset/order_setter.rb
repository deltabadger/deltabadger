  ...
  def set_orders(
    ...
    orders_data.each do |order_data|
      if order_data[:amount].zero?
        Rails.logger.info("set_orders bot=#{id} event=order_ignored #{order_log_fields(order_data)}")
      ...
      if amount_info[:below_minimum_amount]
        Rails.logger.info("set_orders bot=#{id} event=order_skipped #{order_log_fields(order_data)}")
        log_activity('order_skipped', level: :warning, details: order_log_details(order_data))
        create_skipped_order!(order_data)
        next
      end

      Rails.logger.info("set_orders bot=#{id} event=order_creating #{order_log_fields(order_data)}")
      ...
      if result.failure?
        ...
        Rails.logger.info("set_orders bot=#{id} event=order_accepted order_id=#{order_id} #{order_log_fields(order_data)}")
