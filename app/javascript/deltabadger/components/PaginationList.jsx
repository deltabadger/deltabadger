...
export const PaginationList = ({page, setPage, numberOfPages}) => {
  const first = () => {
    if (page === 1)
      return 1
    ...
    <div className="db-bots__item d-flex db-add-more-bots">
      <nav aria-label="bots pagination list">
        <ul className="pagination pagination-lg" style={{marginBottom: 0, paddingBottom: 0}}>
          <li className={firstClass()}><a className="page-link" onClick={() => setPage(first())} tabIndex="-1">{first()}</a></li>
          <li className={secondClass()}><a className="page-link" onClick={() => setPage(second())}>{second()}</a></li>
          { numberOfPages > 2 &&
            <li className={thirdClass()}><a className="page-link" onClick={() => setPage(third())}>{third()}</a></li>
          }
        </ul>
      </nav>
    </div>
