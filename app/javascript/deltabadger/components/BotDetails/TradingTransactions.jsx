...
const TradingStats = ({
  ...
  currentPriceAvailable
    ...
    <table className="table">
      <tbody>
        <tr>
          <td scope="col">{ isBuyingBot(type) ? I18n.t('bot.details.stats.bought') : I18n.t('bot.details.stats.sold')}:</td>
          <th scope="col">{ toFixedWithoutZeros(bought) } { base }</th>
        </tr>
        <tr>
          <td scope="col">{ I18n.t('bot.details.stats.average_price') }:</td>
          <th scope="col">{ toFixedWithoutZeros(averagePrice) } { quote }</th>
        </tr>
        <tr>
          <td scope="col">{ isBuyingBot(type) ? I18n.t('bot.details.stats.total_invested') : I18n.t('bot.details.stats.bought') }:</td>
          <th scope="col">{ toFixedWithoutZeros(totalInvested) } { quote }</th>
        </tr>
        <tr>
          <td scope="col">{ isBuyingBot(type) ? I18n.t('bot.details.stats.current_value') : I18n.t('bot.details.stats.current_value_sold', {base: base})}:</td>
          <th scope="col">
        ...
        <tr>
          <td scope="col">{ I18n.t('bot.details.stats.profit_loss') }:</td>
...
export const TradingTransactions = ({ bot, active }) => {
  ...
  return <div className={`legacy-tab ${active ? 'active' : ''}`} id="statistics" role="tabpanel" aria-labelledby="stats-tab">
    ...
    <p className="db-smallinfo">
      ...
      <RawHTML tag="span">{I18n.t('bot.details.stats.csv_download_html')}</RawHTML>
    </p>
    <div className="csv-download-button">
      <a href={`/api/bots/${bot.id}/transactions_csv`} className="button button--link button--icon-and-text">
      ...
      <span> {I18n.t('bot.details.stats.download_csv')}</span> </a>
    </div>
  </div>
}

const translateBuyOrSell = (side) => {
  return side === 'buy' ? I18n.t('bot.buy') : I18n.t('bot.sell')
}
