...
class ExtendedChartCandlePriceTest < ActiveSupport::TestCase
  ...
  # Two candles because the fetch drops the trailing (still-open) candle.
  def candles(scale: 1)
    [
      [CANDLE_TIME, (100 * scale).to_d, (150 * scale).to_d, (90 * scale).to_d, (140 * scale).to_d, 1.to_d],
      [CANDLE_TIME + 1.day, (140 * scale).to_d, (160 * scale).to_d, (130 * scale).to_d, (155 * scale).to_d, 1.to_d]
    ]
  end
