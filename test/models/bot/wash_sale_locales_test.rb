...
class Bot::WashSaleLocalesTest < ActiveSupport::TestCase
  WASH_SALE_KEYS = %w[title sentence_html option note confirm locked_title locked_none locked_entry
                      selling_warning].freeze
  ...
  # The selling warning's only button. It was missing in eight locales, which showed "Got it!" in English.
  test 'every available_locale translates the acknowledge button' do
    I18n.available_locales.each do |locale|
      data = YAML.load_file(Rails.root.join("config/locales/base.#{locale}.yml"))[locale.to_s]
      assert data.dig('button', 'understand'), "base.#{locale}.yml is missing button.understand"
    end
  end
