...
4. **REVOKE everything it listed, at its source: exchange API keys, fee keys, SMTP
   password, Alpaca key and secret, CoinGecko key, market-data token.** Also regenerate
   your REST API token and revoke any connected MCP clients (Settings → REST API,
   Settings → MCP) — this reset does not touch either. Revoke only — replacements have
   nowhere to live until step 7.
...
8. Sign in, issue fresh credentials, add them, re-enable two-factor, and restart your
   bots and rules — the reset stopped them.
