...
1 lipca 2025 r. SEC [otworzyła nowy rozdział dla inwestycji w kryptowaluty](https://cointelegraph.com/news/sec-conversion-grayscale-large-cap-crypto-fund-etf), zatwierdzając przekształcenie Grayscale Digital Large Cap Fund (GDLC) w fundusz giełdowy (ETF) typu spot, który będzie notowany na NYSE Arca z aktywami pod zarządzaniem o wartości 775 milionów dolarów.
...
<img src="https://deltabadger.com/images/articles/coindesk-methodology.svg" alt="Cztery warstwy Metodologii CoinDesk">
<figcaption class="article__figure__caption">Cztery poziomy Metodologii CoinDesk</figcaption>
</figure>
{:/nomarkdown}
...
<figcaption class="article__figure__caption">Alokacja Indeksu CoinDesk 20 (CD20)</figcaption>
</figure>
{:/nomarkdown}

**Kwartalny rebalancing**: Co trzy miesiące cały portfel jest kalibrowany na nowo w oparciu o bieżące warunki rynkowe, z "zasadami buforowymi" zapobiegającymi nadmiernej rotacji.

Zasady mają na celu zapobieganie nadmiernej rotacji, dając obecnym składnikom indeksu pierwszeństwo w pozostaniu w indeksie, nawet jeśli nieznacznie spadły w rankingach.

CD20 używa **systemu buforowego**:

**Kryteria wejścia**:
1. Aktywo musi znajdować się w top **40** pod względem wolumenu obrotu, aby mogło być brane pod uwagę  
2. **Top 15 miejsc**:  
- Trafia do największych aktywów pod względem kapitalizacji rynkowej (niezależnie od obecnego statusu)  
3. **Pozostałe 5 miejsc**:  
- Obecne składniki z miejsc 16-25 pod względem kapitalizacji rynkowej mają pierwszeństwo  
- Jeśli nie ma wystarczającej liczby obecnych składników, które się kwalifikują, najwyżej notowane nowe aktywa wypełniają pozostałe miejsca  

**Kryteria wyjścia**: Istniejące aktywo traci swoje miejsce, jeśli:  
1. Spadnie poniżej **50.** miejsca pod względem wolumenu obrotu (problem z płynnością), LUB  
2. Spadnie poniżej **25.** miejsca pod względem kapitalizacji rynkowej (problem z wielkością)  

Bez zasad buforowych aktywa wahające się wokół progu byłyby ciągle dodawane i usuwane co kwartał, co generowałoby niepotrzebne koszty transakcyjne i niestabilność.

### CoinDesk 5

Jednak to nie CD20 jest używany w pierwszym wielokryptowalutowym ETF-ie, ale jego młodszy brat:

**CD5** reprezentuje 5 największych aktywów z CD20, wybranych na podstawie **kapitalizacji rynkowej** spośród już sprawdzonych składników CD20. Ponieważ CD20 już stosuje rygorystyczne filtry płynności (w tym screening wolumenu obrotu), CD5 dziedziczy te standardy jakości, koncentrując się na największych, najbardziej płynnych aktywach.

Indeks jest znacznie młodszy i wciąż brakuje mu równego pokrycia online, ale badając jego konstrukcję, jasne jest, że był przeznaczony do użytku przez instytucje.

Kluczową różnicą w stosunku do CD20 jest to, że **limity są usunięte**. Podczas gdy CD20 ogranicza największy składnik do 30%, a pozostałe do 20%, CD5 stosuje czyste ważenie kapitalizacją rynkową. Powoduje to, że Bitcoin osiąga obecnie ponad 80% alokacji — co jest odzwierciedleniem dominującej pozycji rynkowej Bitcoina i jego profilu płynności. Ta koncentracja jest cechą, a nie błędem — zapewnia, że indeks odzwierciedla prawdziwą hierarchię kapitalizacji rynkowej najbardziej płynnych aktywów kryptowalutowych.

Podobnie jak CD20, CD5 stosuje ten sam kwartalny harmonogram rebalancingu, ale używa prostszej **zasady buforowej 4/6**: 4 największe aktywa pod względem kapitalizacji rynkowej z CD20 są automatycznie włączane, podczas gdy istniejące składniki z miejsc 5. lub 6. mają pierwszeństwo w pozostaniu. Ten usprawniony proces sprawia, że CD5 jest szczególnie odpowiedni do wdrożenia ETF, ponieważ zmniejsza złożoność, zachowując jednocześnie rygorystyczne standardy instytucjonalne.
