...
module SecretKeyBaseGuard
  ...
  PUBLISHED_WARNING = <<~MESSAGE.freeze
    ...
    signed into without ever copying the database. Deleting credentials here does NOT
    invalidate them — they must be revoked at their source.
    ...
    In this order:
      ...
      4. REVOKE everything it listed, at its source: exchange API keys, fee keys, SMTP
         password, Alpaca key and secret, CoinGecko key, market-data token. Also
         regenerate your REST API token and revoke any connected MCP clients (Settings
         → REST API, Settings → MCP) — this reset does not touch either. Revoke only —
      ...
      8. Sign in, issue fresh credentials, add them, re-enable two-factor, and restart
         your bots and rules — the reset stopped them.
