...
module SecretKeyBaseGuard
  ...
  PUBLISHED_WARNING = <<~MESSAGE.freeze
    ...
    there. The exception is this instance's own REST API and MCP tokens: they exist only
    in this database, so step 5 does invalidate those.
    ...
    In this order:
      ...
      5. Clear what is stored:
         ...
         what actually invalidates them: they are bearer tokens stored in the clear and
         derived from nothing, so changing the secret alone leaves them trading.
      6. Blank the SECRET_KEY_BASE line in .env.docker.
      7. Start the app. A strong per-install secret is generated for you.
      8. Sign in, issue fresh credentials, add them, re-enable two-factor, re-issue your
         REST API token, reconnect your MCP clients, and restart your bots and rules —
         the reset cleared and stopped all of them.
