class MigrateDualAssetBotsToMultiAsset < ActiveRecord::Migration[8.1]
  ...
  def up
    ...
    # A bot mid-tick is left alone on purpose; the later retirement migration converts whatever is
    # left, whatever its state. The message is for anyone watching a deploy.
    say "#{skipped.size} bot(s) still on the old shape; the retirement migration converts them."
  end
